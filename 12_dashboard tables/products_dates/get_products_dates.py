...
df = pd.read_csv('../data/vend-total_revenue-for-product_variant-by-month.csv')
...
df = pd.read_csv('../data/vend-gross_profit-for-product_variant-by-month.csv')
df = df[[0,3,5]+range(7,len(df.columns)-5)]#reqd columns
df = df[:max(df.index)-4] #reqd rows
df['Product'] = df['Product'].apply(lambda x: x.upper())
df = df.sort('Product')
...
df1 = pd.read_csv('../products_table/products.csv')
